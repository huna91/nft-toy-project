...
contract NFTtoken2 is ERC721Enumerable,Ownable {
    ...
    function getRandomTokenData(address _owner, uint256 _tokenId, userRandomGetNFT storage _userNFT) private returns (uint256) {
        // 임의의 랜덤값을 구한다
        uint256 randomNum = uint256(keccak256(abi.encodePacked(_owner, _tokenId))) % 100;
        ...
        if(userRandomNFTs[msg.sender].randomSecond == false){
            for(uint i = 0; i<60; i++){
                _userNFT.userNFTTokenIndex[_userNFT.userNFTTokenIndex.length] = i;
            }
        }
        else{
            // 토큰의 데이터를 저장할 변수
            // 어차피 여기는 뽑기 구문이기에 length - 1로 미리 자리를 빼놓는다
            for(uint i = 0; i<_userNFT.userNFTTokenIndex.length -1; i++){
                if(randomNum == i){
                    _userNFT.userNFTTokenIndex[i] = _userNFT.userNFTTokenIndex[i+1];
                }    
